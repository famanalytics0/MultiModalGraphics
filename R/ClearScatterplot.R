...
#' @export
ClearScatterplot <- function(data, highLog2fc=0.585, lowLog2fc=-0.585,
...
#'
#' Handles filtering, NA removal, and checks.
#'
...
#' @param var_quantile fraction top by variance
...
ClearScatterplot_MAE <- function(
  ...
  mae_f <- subsetByRow(mae, IRanges::IntegerList(setNames(list(feat),assayName)))
  ...
  if(!length(keep)) stop("No samples after NA filter")
  expr<-expr[,keep,drop=FALSE]; meta<-meta[keep,,drop=FALSE]
  tab<-table(meta[[groupColumn]])
  if(any(tab<3)) stop("<3 samples per group")
  if(ncol(expr)<6) stop("Too few samples")
  .ClearScatterplot_core(expr,meta,groupColumn,sampleType,
...
ClearScatterplot_table <- function(
  ...
  expr<-expr[,keep,drop=FALSE]; meta<-meta[keep,,drop=FALSE]
  tab<-table(meta[[groupColumn]])
  if(any(tab<3)) stop("<3 per group")
  if(ncol(expr)<6) stop("Too few samples")
  .ClearScatterplot_core(expr,meta,groupColumn,
                         sampleType,timepoint,dataType,vectorized,BPPARAM)
}
...
.ClearScatterplot_core <- function(
  ...
  cols<-c(groupColumn,sampleType)
  if(!is.null(timepoint)) cols<-c(cols,timepoint)
  keep<-complete.cases(meta[,cols,drop=FALSE])
  expr<-expr[,keep,drop=FALSE]; meta<-meta[keep,,drop=FALSE]
  if(dataType=="auto") {
    dataType<-if(all(expr==floor(expr))&&max(expr,na.rm=TRUE)>30) "count"
              else "continuous"
  }
  cells<-if(!is.null(timepoint)) expand.grid(
    timePoint=unique(meta[[timepoint]]),
    SampleType=unique(meta[[sampleType]]),stringsAsFactors=FALSE
  ) else data.frame(
    SampleType=unique(meta[[sampleType]]),
    timePoint=NA_character_,stringsAsFactors=FALSE
  )
  run_cell<-function(i) {
    ...
    .run_DE(expr[,idx,drop=FALSE],meta[idx,,drop=FALSE],
            groupColumn,stats::model.matrix(~meta[idx,groupColumn]),
            dataType,list(SampleType=st,timePoint=tp))
  }
  df_list<-if(vectorized=="vectorized"||
              (vectorized=="auto"&&nrow(cells)>BiocParallel::bpworkers(BPPARAM)))
    BiocParallel::bplapply(seq_len(nrow(cells)),run_cell,BPPARAM=BPPARAM) else
    lapply(seq_len(nrow(cells)),run_cell)
  plotdata<-do.call(rbind,df_list)
  if(!nrow(plotdata)) stop("No DE results")
...
setMethod("createPlot","ClearScatterplot",
  function(object,color1="cornflowerblue",
           color2="grey",color3="indianred") {
    df<-object@data;x<-'SampleType';y<-'timePoint'
    ux<-unique(df[[x]]);uy<-unique(df[[y]])
    facet<-if(length(ux)>1&&length(uy)>1) stats::as.formula(paste(y,'~',x))
           else if(length(uy)>1) stats::as.formula(paste(y,'~ .'))
           else stats::as.formula(paste('.~',x))
    ...
    p<-p+ggplot2::geom_text(data=cnt_up,ggplot2::aes(label=n1),
    ...
    object@plot<-p;invisible(object)
})
...
setMethod("show","ClearScatterplot",function(object){
  ...
  print(object@plot);invisible(object)
})
