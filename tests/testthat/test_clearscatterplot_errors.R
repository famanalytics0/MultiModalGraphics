...
test_that("error when missing DE‐data columns", {
  ...
  expect_error(ClearScatterplot(df), "Missing columns")
})
...
test_that("error on mismatched names in list‐of‐matrices", {
  ...
  expect_error(ClearScatterplot(data = list(A=mat), meta = list(B=meta)),
...
test_that("error on missing assayNames/groupColumns in list‐of‐MAEs", {
  ...
  expect_error(ClearScatterplot(data = list(m=mae)),
...
test_that("error on too few samples", {
  ...
  expect_error(ClearScatterplot_table(expr, meta), "Each level of 'Group' must have ≥ 3 samples")
})
