...
test_that("Error: wrong input type", {
  expect_error(InformativeHeatmap(123), "`data`.*not recognized")
})
...
test_that("Error: mismatched names in list-of-matrices", {
  ...
  expect_error(
    InformativeHeatmap(data=list(A=mat1), meta=list(B=meta1), groupColumn="Group"),
...
test_that("Error: missing assayNames/groupColumns for list-of-MAEs", {
  ...
  expect_error(
    InformativeHeatmap(data=list(m=mae)),
